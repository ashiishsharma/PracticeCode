...
public class RobotInAGrid {
    ...
    public ArrayList<Point> getPathDynamicProgramming(boolean[][] grid) {
        if (grid == null || grid.length == 0) return null;
        ArrayList<Point> path = new ArrayList<Point>();
        HashSet<Point> failedPoints = new HashSet<Point>();
        if (getPathDynamicProgramming(grid, grid.length - 1, grid[0].length - 1, path, failedPoints)) {
            return path;
        }
        return null;
    }

    private boolean getPathDynamicProgramming(boolean[][] grid, int row, int col, ArrayList<Point> path,
        ...
        if (isAtOrigin || getPathDynamicProgramming(grid, row, col - 1, path, failedPoints) ||
                getPathDynamicProgramming(grid, row - 1, col, path, failedPoints)) {
            path.add(p);
            return true;
        }
